...
public class NumericMatrix implements Iterable<IndexedEntry<NumericValue>>, Cloneable{
     ...
     * addition of the corresponding entries in this and other matrix.
    ...
    public final NumericMatrix plus(NumericMatrix other) {
        ...
        for (IndexedEntry<NumericValue> e0 : cells) {
            ...
            if (v1 != null)
                ret.fastSet(e0.index(), factory.plus(e0.value(), v1)); // r[i] = M[i] + N[i]
    ...
    /**
     * Returns a new matrix such that an entry in the returned matrix represents the
     * addition of the corresponding entries in this and the other matrices.
    ...
    public final NumericMatrix plus(final NumericMatrix... others) {
        ...
        while(it.hasNext()){
            ...
            ret.fastSet(idx, factory.accumulate(acc));
        }
    ...
    public final NumericMatrix difference(NumericMatrix other) {
        ...
        BiFunction<NumericValue, NumericValue, NumericValue> diff = isB1 && isB2 ?
                ...
                (x1, x2) -> {
                    ...
                    return factory.implies(factory.neq(n1, ZERO), factory.minus(n1, min(n1, n2)));
                };
    ...
    /**
     * Returns a new matrix such that an entry in the returned matrix represents the
     * subtraction of the corresponding entries in this and other matrix.
     * @return this.cells - other.cells
     */
    ...
    /**
     * Returns a new matrix such that an entry in the returned matrix represents the
     * Hadamard division of the corresponding entries in this and other matrix.
    ...
    public final NumericMatrix divide(NumericMatrix other) {
        ...
        for (IndexedEntry<NumericValue> e0 : this.cells) {
            ...
            if (v1 != null)
                ret.fastSet(e0.index(), factory.divide(dividend.apply(e0.value()), divisor.apply(v1)));
            // Division by zero
    ...
    /**
     * Returns a new matrix such that an entry in the returned matrix represents the
     * Hadamard division of the corresponding entries in this and the other matrices.
     *
     * @return this.cells ⊘ others[0].cells ⊘ ... ⊘ others[others.length-1].cells
     */
     ...
     */
    private NumericValue min(NumericValue n0, NumericValue n1){
        if(n0 == ZERO)
            return ZERO;
        ...
        return factory.ite(
                factory.and(factory.neq(n0, ZERO), factory.neq(n1, ZERO)),
                factory.minimum(n0, n1),
                ZERO);
    }
     ...
     */
    private NumericValue max(NumericValue n0, NumericValue n1){
        if(n0 == ZERO)
            return n1;
        ...
        return factory.ite(
                factory.and(factory.neq(n0, ZERO), factory.neq(n1, ZERO)),
                factory.maximum(n0, n1),
                factory.ite(factory.neq(n0, ZERO), n0, n1));
    }

    /**
     * Returns a new matrix such that an entry in the returned matrix represents the
     * intersection of the corresponding entries in this and the other matrix.
     * @return min(this.cells, other.cells)
     */
    public final NumericMatrix intersection(NumericMatrix other){
        ...
        /*else if(isB1 || isB2)
            ...
            intersection = (x1, x2) -> min(toNumeric(x1), toNumeric(x2));
    ...
    public final NumericMatrix union(NumericMatrix other){
        ...
        if(isB1 && isB2)
            ...
            union = (x0, x1) -> max(toNumeric(x0), toNumeric(x1));
     ...
     * @return min(this.cells, other[0].cells, other[1].cells, .., other[n].cells)
     */
    public final NumericMatrix intersection(final NumericMatrix... others) {
        ...
        for (IndexedEntry<NumericValue> cell : cells) {
            ...
            for (NumericMatrix other : others) {
                min = this.min(min, other.fastGet(cell.index()));
                if(min == ZERO)
                    break;
            }
    ...
    public final NumericMatrix union(final NumericMatrix... others) {
        ...
        for (IndexedEntry<NumericValue> cell : cells) {
            ...
            for (NumericMatrix other : others) {
                max = this.max(max, other.fastGet(cell.index()));
            }
            ...
            for(IndexedEntry<NumericValue> cell : others[i])
                if(!visited.contains(cell.index())){
                    ...
                    for (int j = i + 1; j < others.length; j++) {
                        max = this.max(max, others[j].fastGet(cell.index()));
                    }
                    ret.fastSet(cell.index(), max);
                    visited.add(cell.index());
                }
    ...
    public final NumericMatrix cross(final NumericMatrix other) {
        ...
        for (IndexedEntry<NumericValue> e0 : cells) {
            ...
            for (IndexedEntry<NumericValue> e1 : other.cells) {
                NumericValue multiplication = times.apply(e0.value(), e1.value());
     ...
     * @throws IllegalArgumentException dimensions incompatible for multiplication
     */
    public final NumericMatrix dotMinMax(final NumericMatrix other) {
        checkFactory(this.factory, other.factory);
        ...
        if(isB1 && isB2){
            ...
            // n1 + n2 = max(n1, n2)
            addition = (x1, x2) -> max(toNumeric(x1), toNumeric(x2));
            // n1 * n2 = min(n1, n2)
            multiplication = (x1, x2) -> min(toNumeric(x1), toNumeric(x2));
        }
    ...
    public final NumericMatrix closure() {
        ...
        for (int i = 1; i < rowNum; i *= 2) {
            ret = ret.union(ret.dotMinMax(ret));
        }
    ...
    public final NumericMatrix reflexiveClosure(List<BooleanValue> fpEq) {
        ...
        if(cells.isEmpty()){
            ...
            // x11 = *(a11 + a12.*a22.a21)
            NumericMatrix x11 = a11.union(a12.dotMinMax(a22star).dotMinMax(a21)).reflexiveClosure(fpEq);
            assignClosure.apply(0, x11);
            // x22 = *(a22 + a21.*a11.a12)
            NumericMatrix x22 = a22.union(a21.dotMinMax(a11star).dotMinMax(a12)).reflexiveClosure(fpEq);
    ...
    public final NumericMatrix drop() {
        ...
        for (IndexedEntry<NumericValue> e0 : cells) {
            ret.cells.put(e0.index(), factory.dropNum(e0.value()));
        }
    ...
    public final NumericMatrix cardinality() {
        return factory.constant(dims, Ints.rangeSet(Ints.range(0, dims.capacity() - 1)),
                factory.plus(cells.values().toArray(new NumericValue[cells.size()])));
    }
    ...
    public final BooleanValue one(){
        return  // Cardinality returns a constant matrix and thus, the first value stored contains the resulting numeric value
                factory.eq(this.drop().cardinality().getFirst(), ONE);
    }
    ...
    public final BooleanValue lone(){
        return  // Cardinality returns a constant matrix and thus, the first value stored contains the resulting numeric value
                factory.lte(this.drop().cardinality().getFirst(), ONE);
    }

    /**
     * Returns a formula specifying that this matrix is left stochastic, i.e.,
     * it is a square matrix and each column adds up to 1.
     * TODO: Currently checks only the non-zero columns, due to the nature of the sparse sequence.
     * @return this.isSquare() & sum column(this, 0) = 1 & sum column(this, 1) = 1 & .. & sum column(this, n) = 1
     */
    /*public final BooleanValue isLeftStochastic(){
        if(!dims.isSquare() || (dims.capacity() > 0 && cells.isEmpty()))
            return FALSE;

        int n = dims.dimension(0);
        // <column, cells in that column>
        Map<Integer, List<NumericValue>> columns = new HashMap<>();

        // Group this matrix cells by columns
        for(IndexedEntry<NumericValue> cell : cells){
            int c = cell.index() % n;
            if(!columns.containsKey(c))
                columns.put(c, new ArrayList<>());
            columns.get(c).add(cell.value());
        }

        BooleanValue isLS = TRUE;
        for(List<NumericValue> col : columns.values()){
            if(col.size() > 0)
                isLS = factory.and(isLS,
                        factory.eq(
                                factory.plus(col.toArray(new NumericValue[0])), // sum col
                                NumericConstant.ONE                             // = 1
                        ));
        }
     ...
     * Returns a formula specifying that this matrix is left stochastic, i.e.,
     * it is a square matrix and each column adds up to 1.
     * TODO: Assumes that there are no relevant columns full of zeros.
     *
     * @return this.isSquare() & this . ! = !
     */
    public final BooleanValue isLeftStochastic(){
        if(!dims.isSquare() || (dims.capacity() > 0 && cells.isEmpty()))
            return FALSE;

        int[] n = new int[]{ dims.dimension(0) };
        // Determine all the non-zero columns
        int cap = dims.capacity();
        Set<Integer> columns = new TreeSet<>();

        Iterator<IndexedEntry<NumericValue>> it = cells.iterator();
        boolean isVector = true;
        while(it.hasNext() && isVector){
            columns.add(it.next().index() % n[0]);
            isVector = columns.size() == 1;
        }

        BooleanValue isLS;
        if(isVector){
            // vector => is it a distribution ?
            isLS = factory.eq(this.sum().getFirst(), ONE); // Sum returns a constant matrix
        }else{
            // !
            final NumericMatrix bang = new NumericMatrix(Dimensions.rectangular(n), factory, cells, cells);
            for(int i = 0; i < n[0]; i++)
                bang.set(i, ONE);

            // this . ! = !
            isLS = this.dot(bang).eq(ONE);
        }
        return isLS;
    }

    /**
     * Transforms this matrix into a left-stochastic matrix by normalizing,
     * preserving the ratio of its weights.
     * If this matrix already is left-stochastic, it has no effect.
     * @return Normalization of this
     *  all distribution : this | norm(distribution) = distribution / sum(distribution)
     *  TODO: Vector normalization
     */
    public final NumericMatrix normalize(){
        final NumericMatrix ret = new NumericMatrix(dims, factory, cells, cells);
        int n = dims.dimension(0);
        Map<Integer, List<NumericValue>> columns = new HashMap<>();
        int col;

        // Fill the mapping between each column and its elements
        for (IndexedEntry<NumericValue> e0 : cells) {
            col = e0.index() % n;
            if(columns.containsKey(col))
                columns.get(col).add(e0.value());
            else{
                List<NumericValue> colValues = new ArrayList<>();
                colValues.add(e0.value());
                columns.put(col, colValues);
            }
        }

        // Compute the sum of each column
        Map<Integer, NumericValue> colSum = columns.entrySet()
                .stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        c -> factory.plus(c.getValue().toArray(new NumericValue[0]))));

        // Define the normalized matrix
        for (IndexedEntry<NumericValue> e0 : cells) {
            col = e0.index() % n;
            ret.cells.put(e0.index(), factory.divide(e0.value(), colSum.getOrDefault(col, ZERO)));
        }
