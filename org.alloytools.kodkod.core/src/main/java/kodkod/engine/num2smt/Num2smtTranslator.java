...
/**
 * Transforms an integer problem into a SMT problem as a SMT-LIB2 specification, with certain characteristics:
 * - each function symbol has a unique identifier between fs of the same sort
 * - the specification produced will be composed exclusively by integer and boolean function symbols,
 * following a naming convention such that, if a boolean fs X and an integer fs B have the same identifier,
 * then the boolean fs represents the integer fs from the boolean pov, i.e., B(i) = X(i) > 0
 * - Integer fs may or may not be bounded by a {@code limit}. In case such limit exists, integer fs representing
 ...
 *  limit = null => unbounded integer fs
 * @specfield intFS : [Integer, String]
 * intFS associates the identifiers of the integer fs to its variable name.
 */
public class Num2smtTranslator implements BooleanVisitor<Integer,Object>, NumericVisitor<Integer, Object> {
    ...
    // Integer Function Symbols declared
    protected final Map<Integer, String> intFS;
     ...
     */
    public int getNumberOfVariables(){ return this.intFS.size() + this.boolFS.size(); }
    ...
    public static SMTSolver translate(final Collection<BooleanFormula> problem, QuantitativeOptions options){
        final Num2smtTranslator translator = new Num2smtTranslator(options, "QF_NIA");
    ...
    /**
     * Declares an integer function symbol {@code var} with identifier {@code id}, s.t.,
     ...
     * @param extend If extend = true and there already is an integer fs declared with
    ...
    protected String addVariable(int id, String var, Integer minValue, Integer maxValue, boolean extend){
        // Declaration
        if(intFS.containsKey(id))
            var = intFS.get(id);
        else declare("Int", var);

        if(!intFS.containsKey(id) || extend){
            intFS.put(id, var);
    ...
    /**
     * Introduces a new unbound integer variable.
     */
    ...
    /**
     * Obtain the integer variable with the given label.
     * If there is none, a new unbound integer variable will be created.
     */
    private String getNumeric(int v){
        if(!intFS.containsKey(v) && binaryValues.containsKey(v)){
    ...
    /**
     * Obtain a collection of the integer variables identified
    ...
    private String addBoolVariable(int b){
        ...
        if(!boolFS.containsKey(b)){
            ...
            if(intFS.containsKey(b))
                addAssertion(String.format("(= %s (not (= %s 0)))", var, intFS.get(b)));
    ...
    /**
     * Defines the comparison between an integer variable and a constant integer value.
     */
    ...
    /**
     * Specifies an ITEGate over boolean or integer values.
    ...
    /**
     * Adds the reference to a integer constant on the assertion stack.
