...
public class A4QtSolutionReader extends A4SolutionReader {
    ...
    protected TupleSet parseTuples(XMLNode tuples, int arity) throws Err {
        ...
        // Weight Parser
        Function<String, Number> parseNum = n ->
                Pattern.matches("(-?)[0-9]+(\\.[0-9]+)?([eE][+-]?[0-9]+)?", n) ?
        ...
        if(it.hasNext()){
            ...
            if(n.is("tuple")) {
                ...
                if (isBoolean) {
                    ...
                    ((QtTupleSet) ans).add(t.a, parseNum.apply(t.b));
                }
                ...
                while (it.hasNext())
                    if ((n = it.next()).is("tuple")) {
                        ...
                        if (isBoolean)
                            ans.add(t.a);
                        else ((QtTupleSet) ans).add(t.a, parseNum.apply(t.b));
                    }
            }
            // Empty
    ...
    protected TupleSet parseAtom(XMLNode sub, TupleSet atoms){
        int w = Integer.parseInt(sub.getAttribute("value", "1"));
